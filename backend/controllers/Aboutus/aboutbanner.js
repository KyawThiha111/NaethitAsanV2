...
export const UpdateAboutBanner = async (req, res) => {
  try {
    ...
    const requiredFields = {
      ...
      visionquoteen, visionquotemy,
      visiontexten, visiontextmy
    };
    ...
    if (req.files?.bannerbgimg) {
      ...
      if (existingBanner.bannerbgimg) {
        const oldPath = path.join(__dirname, "..","..", "public", existingBanner.bannerbgimg);
    ...
    if (req.files?.backgroundblogimg) {
      ...
      if (existingBanner.backgroundblogimg) {
        const oldPath = path.join(__dirname, "..","..","public", existingBanner.backgroundblogimg);
        if (fs.existsSync(oldPath)) {
          fs.unlinkSync(oldPath);
        }
      }
...
export const GetAboutBanner = async (req, res) => {
  try {
    ...
    const bannerbgimgUrl = banner.bannerbgimg ? `${BASE_URL}${banner.bannerbgimg}` : null;
    const backgroundblogimgUrl = banner.backgroundblogimg ? `${BASE_URL}${banner.backgroundblogimg}` : null;
    ...
    const response = {
      ...
      introduction: lang === 'en' ? banner.introductionen : banner.introductionmy,
      vision: {
